function PTPfigures(task, units)
...
% get data
alllow = zeros(numSub,4); %matrix for recording which ses is low
allbaselinef0_lowhigh = zeros(numSub,4); %matrix for all baselines
allgender = cell(numSub,1); % ???

for s = 1:numel(subIDs)
    conn_loadmatfile(fullfile(derivDir, sprintf('%s', subIDs{s}), sprintf('%s_task-%s_desc-sessioninfo.mat', subIDs{s}, task)), 'LOW', 'baselinef0', 'gender'); %load baseline and h/l info
    alllow(s, :) = LOW; %Load LOW value into matrix for sub
    baselinef0_lowhigh = horzcat(baselinef0(LOW), baselinef0(~LOW)); %Load baselines. Column 1: low Column 2: high
    allbaselinef0_lowhigh(s, :) =  baselinef0_lowhigh; %Load all baselines
    allgender{s} = gender;
end
...
% figure
h = figure(); h.Units = 'pixels'; h.Position = [500 500 500 600]; %Initialize figure
hold on
for s = 1:numSub
    plot(meanbaselinef0(s,:)', '-gs','linewidth', 2.5, 'MarkerSize',5,'Color',colorVec{s})
end
ylim([min(min(meanbaselinef0))-20,max(max(meanbaselinef0))+20])
xlim([0.5,2.5])
ylabel('f_{o} (Hz)', 'FontSize', 16)
set(gca,'XTickLabel', {'','Low', '','High'},'FontSize', 16)
title('Baseline f_{o}', 'FontSize', 20)

% save
figFile = fullfile(derivDir, 'ajacosta-thesis', 'fig-1_sub-all_desc-baselinef0-lowhigh-lineplot.jpg');
