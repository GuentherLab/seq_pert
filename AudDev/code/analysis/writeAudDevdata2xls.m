function writeAudDevdata2xls(dirname,sData,f0F1only)
% function writeAudDev2xls(dirname,strct)
%   inputs: dirname: directory where excel files will be stored
%           strct: AudDev subject data structure (traces aggregated within condition
%                   across runs 
...
for cIdx = 1:size(cCell,1)
    ...
    %Iterate through each trace type and generate a new csv file for
    %each - to make csv files
%     for tIdx = 1:size(tData, 1)
%         csvfilename = sprintf('validTrials_%s_f%u.csv', cCell{cIdx,1}, tIdx-1);
%         csvfile = fullfile(dirname, csvfilename);
%         % If csv file exists, delete and replace
%         if isfile(csvfile)
%             delete(csvfile);
%         end
%         T = table(tData{tIdx,2});
