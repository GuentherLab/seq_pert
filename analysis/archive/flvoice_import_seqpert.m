...
function varargout=flvoice_import_seqpert(SUB,SES,RUN,TASK, varargin)
        ...
        if OPTIONS.OVERWRITE||~conn_existfile(filename_fmtData)
            for trialNum=modifytrials(:)'
                ...
                for ns=1:numel(out_trialData(trialNum).dataLabel), 
                    ...
                    %{
                    if isfield(in_trialData,'reference_time')
                        in_trialData = rmfield(in_trialData,'reference_time');
                    end
                    %}
                    
                    % swiched order of if statement and first elseif statement
                    if isfield(in_trialData(trialNum),'timingTrial'), pertOnset = in_trialData(trialNum).timingTrial(4)-in_trialData(trialNum).timingTrial(2);
                    elseif isfield(in_trialData(trialNum),'reference_time'), pertOnset = in_trialData(trialNum).reference_time - t0{ns}; % note: pertOnset relative to beginning of audio sample
